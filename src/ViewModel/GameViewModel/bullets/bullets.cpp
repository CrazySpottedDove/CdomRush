...
void Bullet::check_position(Store& store)
{
    ...
    if (source_type == SourceType::Tower || source_type == SourceType::Soldier) {
        ...
        if (target == nullptr || target->animations[0].current_state == State::Death)
            target_alive = false;   // 如果目标死亡，则不再更新
        else
            target_position = target->position + target->Hit_offset;   // 更新目标位置
    }
...
void Arrow::Update(Store& store)
{
    ...
    sf::Vector2f dBezier =
        2.0f * (1.0f - t) * (p1 - source_position) + 2.0f * t * (target_position - p1);
...
void Bomb::Update(Store& store)
{
    ...
    if (t >= 1.0f) {
        ...
        else if (source_type == SourceType::Enemy) {
