    ...
    //, initialized_(false)
{
    ...
    DEBUG_CODE(std::cout << "BulletUI created for bullet at (" << bullet_->position.x << "," << bullet_->position.y 
 ...
 * @brief 渲染Bullet
 */
void BulletUI::Render(sf::RenderWindow& window, const Position& position,Animation& animation,const sf::Vector2f& scale)
{
    if (bullet_ == nullptr) {
        return;
    }
    ...
    if (animation.state == State::Hit && animation_player_.IsAnimationFinished(animation, animation_context_)) {
        return;
    }
...
//     DEBUG_CODE(std::cout << "BulletUI: Initialized animation for " << bullet_->animation.prefix 
...
//         DEBUG_CODE(std::cout << "BulletUI: State changed for " << bullet_->animation.prefix 
