...
void Mage::layer_update() {
    if(animation.state == State::Idle && heading == tower_heading::Down){
        Layers[2].animation.state = State::Idle; // 设置朝下的动画状态
        Layers[3].animation.state = State::IdleDown; // 设置朝下的动画
    }
    else if(animation.state == State::Idle && heading == tower_heading::Up){
        Layers[2].animation.state = State::Idle; // 设置朝上的动画状态
        Layers[3].animation.state = State::IdleUp; // 设置朝上的动画
    }
    else if(animation.state == State::Shoot && heading == tower_heading::Down){
        Layers[2].animation.state = State::Shoot; // 设置射击朝下的动画状态
        Layers[3].animation.state = State::ShootingDown; // 设置射击朝下的动画
    }
    else if(animation.state == State::Shoot && heading == tower_heading::Up){
        Layers[2].animation.state = State::Shoot; // 设置射击朝上的动画状态
        Layers[3].animation.state = State::ShootingUp; // 设置射击朝上的动画
    }
}
