...
{
    DEBUG_CODE(std::cout << "AnimationPlayer initialized (stateless design with AnimationContext)"
                         << std::endl;)
}
...
void AnimationPlayer::RenderAnimation(sf::RenderWindow& window, const sf::Vector2f& position,
                                      Animation& animation, const sf::Vector2f& scale)
{
    try {
        const AnimationGroup current_animation_group =
            animation_manager_.RequireAnimationGroup(animation.prefix, animation.state);

        // 新的状态，重新设置起始帧，并标志动画组正在进行
        if (animation.last_state != animation.state) {
            animation.frame_id = current_animation_group.from;
            animation.pending  = true;
        }
        // 上一组动画播放完毕，重置起始帧，并标志动画组正在进行
        else if (animation.frame_id > current_animation_group.to) {
            animation.frame_id = current_animation_group.from;
            animation.pending  = true;
        }
        ...
        // 2. 请求纹理
        const auto& [frame_data, texture] =
            animation_manager_.RequireFrameData(animation.prefix, animation.frame_id);
        ...
        sprite.setOrigin(sf::Vector2f(
            frame_data.displaySize.x * animation.anchor_x - frame_data.trim_left,
            frame_data.displaySize.y * (1 - animation.anchor_y) - frame_data.trim_top));

        // 5. 处理翻转
        sf::Vector2f final_scale = scale;
        if (animation.flip) {
            final_scale.x = -scale.x;   // 翻转X轴
            DEBUG_CODE(std::cout << "Animation flipped for prefix: " << animation.prefix
                                 << std::endl;)
        }
        ...
        // 当前动画组播放完毕，标志它
        if (animation.frame_id > current_animation_group.to) {
            animation.pending = false;
        }

        DEBUG_CODE(std::cout << "RenderAnimation - prefix: " << animation.prefix
                             << ", state: " << static_cast<int>(animation.state) << ", frame: "
                             << animation.frame_id << ", flip: " << animation.flip << std::endl;)
    }
...
//         DEBUG_CODE(std::cout << "Updating animation state from "
//                              << static_cast<int>(animation.last_state) << " to "
//                              << static_cast<int>(animation.state) << std::endl;)
...
//         if (total_frames == 0) {
//             std::cerr << "Warning: Animation has 0 frames for prefix: " << animation.prefix
//                       << std::endl;
//             animation.pending = false;
//             return;
//         }
...
//                 DEBUG_CODE(std::cout << "Animation looped for prefix: " << animation.prefix
//                                      << std::endl;)
//             }
//             else {
//                 animation.frame_id = total_frames - 1;   // 停止在最后一帧
//                 animation.pending  = false;              // 动画结束
//                 DEBUG_CODE(std::cout << "Animation finished for prefix: " << animation.prefix
//                                      << std::endl;)
//             }
//         }
...
//  * @brief 统一的播放接口
//  */
// void AnimationPlayer::PlayAnimationState(Animation& animation, AnimationContext& context, bool loop)
// {
...
//         should_reset = true;
//         DEBUG_CODE(std::cout << "Switching animation from "
//                              << static_cast<int>(animation.last_state) << " to "
//                              << static_cast<int>(animation.state) << std::endl;)
//     }
 ...
 * @brief 渲染地图背景
 */
void AnimationPlayer::RenderMap(sf::RenderWindow& window, const std::string& map_prefix,
                                const sf::Vector2f& position, const sf::Vector2f& scale) const
{
    try {
        // 地图被当作静态动画：状态=Idle，获取动画组信息
        const auto& animation_group =
            animation_manager_.RequireAnimationGroup(map_prefix, State::Idle);

        // 使用动画组的起始帧（对于地图来说，from和to都是1）
        std::size_t actual_frame_index = animation_group.from;

        const auto& [frame_data, texture] =
            animation_manager_.RequireFrameData(map_prefix, actual_frame_index);
        ...
        // 调试信息（每60帧输出一次）
        DEBUG_CODE(static int debug_counter = 0; debug_counter++; if (debug_counter % 60 == 0) {
            std::cout << "Map rendered: " << map_prefix << " at (" << position.x << ","
                      << position.y << ")"
                      << " scale (" << scale.x << "," << scale.y << ")"
                      << " frame_index (" << actual_frame_index << ")"
                      << " size (" << frame_data.frameRect.size.x << "x"
                      << frame_data.frameRect.size.y << ")" << std::endl;
        })
    }
    catch (const std::exception& e) {
...
//  */
// std::size_t AnimationPlayer::CalculateActualFrameIndex(const Animation& animation,
//                                                        State            state) const
// {
//     try {
//         const auto& animation_group =
//             animation_manager_.RequireAnimationGroup(animation.prefix, state);
...
//  * @brief 检查指定的帧ID是否有效
//  */
// bool AnimationPlayer::IsValidFrameId(const std::string& prefix, State state,
//                                      std::size_t frame_id) const
// {
...
//  */
// void AnimationPlayer::PlayAnimation(Entity& entity, AnimationContext& context, State state, bool
// loop)
// {
//     // 使用指定的state
//     bool should_reset = false;
...
//         should_reset = true;
//         DEBUG_CODE(std::cout << "Starting new animation for Entity prefix: " <<
//         entity.animation.prefix
//                   << " (state: " << static_cast<int>(state) << ")" << std::endl;)
//     } else if (context.needs_state_change(state)) {
//         should_reset = true;
//         DEBUG_CODE(std::cout << "Switching Entity animation from " <<
//         static_cast<int>(context.current_state)
...
//     try {
//         std::size_t total_frames = GetAnimationFrameCount(entity.animation.prefix,
//         context.current_state);

//         if (total_frames == 0) {
//             std::cerr << "Warning: Entity animation has 0 frames for prefix: " <<
//             entity.animation.prefix << std::endl; entity.animation.pending = false; return false;
//         }
...
//             if (context.loop_enabled) {
//                 entity.animation.frame_id = 0;
//                 DEBUG_CODE(std::cout << "Entity animation looped for prefix: " <<
//                 entity.animation.prefix << std::endl;) return true;
//             } else {
//                 entity.animation.frame_id = total_frames - 1;
//                 entity.animation.pending = false;
//                 DEBUG_CODE(std::cout << "Entity animation finished for prefix: " <<
//                 entity.animation.prefix << std::endl;) return false;
//             }
//         }
...
//  */
// void AnimationPlayer::Render(sf::RenderWindow& window, const Entity& entity, const
...
//     try {
//         std::size_t actual_frame_index = CalculateActualFrameIndex(entity.animation,
//         context.current_state);

//         const auto& [frame_data, texture] =
//         animation_manager_.RequireFrameData(entity.animation.prefix, actual_frame_index);
...
//         // displaySize信息
//         DEBUG_CODE(std::cout << "Entity render - displaySize: " << frame_data.displaySize.x <<
//         "x" << frame_data.displaySize.y
//                   << ", frameRect: " << frame_data.frameRect.size.x << "x" <<
//                   frame_data.frameRect.size.y
...
//     DEBUG_CODE(std::cout << "Entity animation stopped for prefix: " << entity.animation.prefix <<
//     std::endl;)
// }
...
//     DEBUG_CODE(std::cout << "Entity animation reset to frame 0 for prefix: " <<
//     entity.animation.prefix << std::endl;)
// }
...
//  * @brief 检查动画是否播放完成（Entity版本）
//  */
// bool AnimationPlayer::IsAnimationFinished(const Entity& entity, const AnimationContext& context)
// const
// {
...
//     try {
//         std::size_t total_frames = GetAnimationFrameCount(entity.animation.prefix,
//         context.current_state); return entity.animation.frame_id >= total_frames - 1;
//     } catch (const std::exception& e) {
//         std::cerr << "Error checking Entity animation finish status: " << e.what() << std::endl;
//         return true;
//     }
...
//  */
// bool AnimationPlayer::JumpToFrame(Entity& entity, AnimationContext& context, std::size_t
// frame_id)
// {
//     try {
...
//         should_reset = true;
//         DEBUG_CODE(std::cout << "Starting new animation for Unit prefix: " <<
//         unit.animation.prefix
...
//         should_reset = true;
//         DEBUG_CODE(std::cout << "Switching Unit animation from " <<
//         static_cast<int>(context.current_state)
...
//     try {
//         std::size_t total_frames = GetAnimationFrameCount(unit.animation.prefix,
//         context.current_state);

//         if (total_frames == 0) {
//             std::cerr << "Warning: Unit animation has 0 frames for prefix: " <<
//             unit.animation.prefix << std::endl; unit.animation.pending = false; return false;
//         }
...
//             if (context.loop_enabled) {
//                 unit.animation.frame_id = 0;
//                 DEBUG_CODE(std::cout << "Unit animation looped for prefix: " <<
//                 unit.animation.prefix << std::endl;) return true;
//             } else {
//                 unit.animation.frame_id = total_frames - 1;
//                 unit.animation.pending = false;
//                 DEBUG_CODE(std::cout << "Unit animation finished for prefix: " <<
//                 unit.animation.prefix << std::endl;) return false;
//             }
//         }
...
//  * @brief 渲染当前帧（Unit版本）
//  */
// void AnimationPlayer::Render(sf::RenderWindow& window, const Unit& unit, const AnimationContext&
// context,
//                             const sf::Vector2f& scale)
// {
//     if (!unit.animation.pending) {
...
//     try {
//         std::size_t actual_frame_index = CalculateActualFrameIndex(unit.animation,
//         context.current_state);

//         const auto& [frame_data, texture] =
//         animation_manager_.RequireFrameData(unit.animation.prefix, actual_frame_index);
...
//         // displaySize和翻转
//         DEBUG_CODE(std::cout << "Unit render - displaySize: " << frame_data.displaySize.x << "x"
//         << frame_data.displaySize.y
//                   << ", frameRect: " << frame_data.frameRect.size.x << "x" <<
//                   frame_data.frameRect.size.y
//                   << ", trim: (" << frame_data.trim_left << "," << frame_data.trim_top
...
//     DEBUG_CODE(std::cout << "Unit animation stopped for prefix: " << unit.animation.prefix <<
//     std::endl;)
// }
...
//     DEBUG_CODE(std::cout << "Unit animation reset to frame 0 for prefix: " <<
//     unit.animation.prefix << std::endl;)
// }

// /**
//  * @brief 检查动画是否播放完成（Unit版本）
//  */
// bool AnimationPlayer::IsAnimationFinished(const Unit& unit, const AnimationContext& context)
// const
// {
...
//     try {
//         std::size_t total_frames = GetAnimationFrameCount(unit.animation.prefix,
//         context.current_state); return unit.animation.frame_id >= total_frames - 1;
//     } catch (const std::exception& e) {
