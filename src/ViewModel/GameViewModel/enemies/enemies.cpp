...
void PassiveEnemy::Update(Store& store)
{
    ...
    if (this->health.hp <= 0) {
        this->animation.current_state = State::Death;   // 如果生命值为0，进入死亡状态
        death_action();   // 执行死亡行为
    ...
    animation.current_state = walkjudge();   // 根据当前方向设置状态
    return;
}
...
void ActiveEnemyMelee::Update(Store& store)
{
    ...
    if (this->animation.current_state == State::Idle) {
        heading = Heading::Right;   // 设置方向为向右
        Soldier* Blocker = store.GetSoldier(this->blocker);   // 获取阻挡单位
        if (Blocker == nullptr) {
            this->animation.current_state = walkjudge();
            return;
        }
        if (Blocker->animation.current_state == State::Death) {
    ...
    if (is_moving()) {                         // 如果敌人正在移动
        calc::enemy_move_tick(store, *this);   // 调用运动函数更新位置
        if (blocker != INVALID_ID) animation.current_state = State::Idle;   // 如果有阻挡单位，设置状态为闲置
        else animation.current_state = walkjudge();         // 根据当前方向设置状态
        return;
    }
    else if (this->animation.current_state == State::Attack) {
...
void ActiveEnemyRange::Update(Store& store)
{
    ...
    if(animation.current_state == State::Idle) {
        ...
        if(Blocker == nullptr){
            ...
            this->animation.current_state = walkjudge();   // 如果没有阻挡单位，也没有射击目标，那就设置状态为行走
            return ;
        }
        if(Blocker->animation.current_state == State::Death){
            blocker = INVALID_ID;
        ...
        if(blocker!=INVALID_ID) animation.current_state = State::Idle;
        else if(!shoot_judge(store)) animation.current_state = walkjudge();
        return ;
    }
    ...
    else if (this->animation.current_state == State::Shoot) {
        if(animation.pending == true) return ;
        animation.current_state = State::Idle;
        animation.pending = true;   // 设置动画为进行中
        return;
...
ForestTroll::ForestTroll(Position position_)
{
    ...
    this->animation.current_state = State::Idle;          // 设置初始状态
    ...
    this->animation.current_state = State::Idle;   // 设置动画状态为闲置
